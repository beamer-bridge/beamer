...
export function handleLPAdded(event: LPAddedEvent): void {
  let entity = new LPAdded(event.transaction.hash.toHex() + '-' + event.logIndex.toString());
  entity.lp = event.params.lp;
  entity.save();
}

export function handleLPRemoved(event: LPRemovedEvent): void {
  let entity = new LPRemoved(event.transaction.hash.toHex() + '-' + event.logIndex.toString());
  entity.lp = event.params.lp;
  entity.save();
}
...
export function handleRequestFilled(event: RequestFilledEvent): void {
  let entity = new RequestFilled(event.transaction.hash.toHex() + '-' + event.logIndex.toString());
