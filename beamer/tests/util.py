...
def update_token(request_manager, token, params, *args, **kwargs):
    ...
    for i, key in enumerate(["transfer_limit", "min_lp_fee", "lp_fee_ppm", "protocol_fee_ppm"]):
        token_data[i] = params.get(key, token_data[i])
    # the last element of token data is collectedProtocolFees which is not part of the update
    request_manager.updateToken(token.address, *token_data[:-1], *args, **kwargs)
